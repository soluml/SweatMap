  ...
  constructor(obj) {
    ...
    if (typeof obj.existing_strings == "object") {
      Object.keys(obj.existing_strings).forEach(key => {
        if (
          typeof key == "string" &&
          typeof obj.existing_strings[key] == "string"
        ) {
    ...
    Object.keys(Ranges).forEach(CR => {
      if (Ranges[CR].start == undefined || Ranges[CR].end == undefined) return;

      for (
        let i = parseInt(Ranges[CR].start, 16);
        i <= parseInt(Ranges[CR].end, 16);
        i++
      ) {
        try {
          ...
          if (!this.characters[bytes]) this.characters[bytes] = [];
  ...
  generatePatternForBytes(bytes) {
    ...
    const calculateSumCombinations = n => {
      ...
      const subsetSum = (subset, target, partial = []) => {
        ...
        subset.forEach(num =>
          subsetSum(subset.slice(1), target, partial.concat(num))
        );
      };
    ...
    return uniq(calculateSumCombinations(bytes), isEqual).map(pattern =>
      pattern.map(val => this.characters[`${val}`])
    );
  }

  set(key) {
    //If it's already been done, don't do it again!
    if (this.fmap.has(key)) return this.fmap.get(key);
    ...
    const isGoodValue = value => {
      ...
      else return !this.has_obfuscated(value);
    };
    ...
    do {
      ...
      while (!isGoodValue(value) && patternsCounter < patterns.length) {
        ...
        //Last possible match
        let lastMatch = currentPattern
          .map(charlist => charlist[charlist.length - 1])
          .join("");
        ...
        while (!isGoodValue(value) && lastMatch != value) {
          ...
          for (let i = 0, ln = currentPatternCounters.length; i < ln; i++) {
            ...
            if (currentPatternCounters[i] <= currentPattern[i].length) {
              ...
              if (
                currentPatternCounters[i] == currentPattern[i].length &&
                i < ln - 1
              ) {
...
if (typeof window === "object") window.SweatMap = SweatMap;

//... and CommonJS
if (typeof module === "object" && module.exports) module.exports = SweatMap;
